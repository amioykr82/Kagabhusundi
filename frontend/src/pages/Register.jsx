...
import { Mail, Lock, User, Eye, EyeOff, Star, Sparkles, Calendar, ArrowLeft, Phone, CheckCircle } from 'lucide-react'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success('Account created successfully! Welcome to Kagabhushundi!')
        ...
        <div className="glassmorphism-card p-8">
          ...
          {registrationSuccess ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Welcome to Kagabhushundi!</h3>
              <p className="text-purple-200 mb-4">Your account has been created successfully.</p>
              <p className="text-purple-300 text-sm">You are now logged in and will be redirected shortly...</p>
              <div className="mt-6">
                <Link 
                  to={from}
                  className="btn-primary px-6 py-3 rounded-lg transition-all duration-300 hover:scale-105"
                >
                  {from === '/' ? 'Continue to Home' : 'Continue to Dashboard'}
                </Link>
              </div>
            </div>
          ) : (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-purple-300 mt-1">Password must be at least 8 characters with uppercase, lowercase, and digit</p>
            </div>
